...
for i = 1:nStations
    ...
    % Standardizzazione manuale delle X
    mu = mean(X_train{:,:}, 1);
...
for i = 1:nStations
    res = residuals{i};              % {} per cell array
    res = res(~isnan(res));
    if numel(res) > 10
        acf = autocorr(res, 'NumLags', 1);   % res è già colonna, no trasposta
        acf1(i) = acf(2);
    else
        acf1(i) = NaN;
    end
